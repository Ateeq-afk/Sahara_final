...
class BlogFileStore {
  ...
  constructor() {
    this.dataFile = path.join(process.cwd(), 'data', 'blog-posts.json')
    this.posts = new Map()
  }
  ...
  private async loadPosts() {
    ...
    try {
      ...
      // If file doesn't exist, initialize with sample posts
      // console.log('Initializing with sample posts')
  ...
  async getPost(idOrSlug: string) {
    ...
    // console.log('Getting post with ID or slug:', idOrSlug)
    // console.log('Total posts in store:', this.posts.size)
    
    // Check if it's an ID
    let post = this.posts.get(idOrSlug)
    if (post) {
      // console.log('Found post by ID:', post.id)
    }
    
    // If not found by ID, search by slug
    if (!post) {
      const allPosts = Array.from(this.posts.values())
      // console.log('Searching by slug. All slugs:', allPosts.map(p => p.slug))
      post = allPosts.find(p => p.slug === idOrSlug)
      if (post) {
        // console.log('Found post by slug:', post.slug)
      }
    }
    ...
    // console.log('Post not found for:', idOrSlug)
    return null
  }
...
const blogFileStore = new BlogFileStore()
